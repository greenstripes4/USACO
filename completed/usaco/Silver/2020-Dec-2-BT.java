...
public class Main {
    ...
    private static ArrayList<Integer> isValid(int[][] cows, boolean[] used, int minX, int maxX, int minY, int maxY) {
    ...
    private static void dfs(int[][] cows, boolean[] used, int minX, int maxX, int minY, int maxY, int index) {
        ArrayList<Integer> invalid = isValid(cows, used, minX, maxX, minY, maxY);
        for(int i: invalid) {
            if(i < index) {
                return;
            }
