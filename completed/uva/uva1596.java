...
class ArrayPlaceHolder{
    int size;
    HashMap<Integer,Integer> assigned;
    public ArrayPlaceHolder(int s, HashMap<Integer,Integer> pairs){
        size = s;
        assigned = pairs;
    }
}
...
public class Main{
    public static int getIndex(String command, HashMap<Character,ArrayPlaceHolder> map){
        if(command.contains("[") && command.contains("]")){
            ...
            return map.get(name).assigned.getOrDefault(nextInd,-1);
        }
    ...
    public static boolean check(String[] assignment, HashMap<Character,ArrayPlaceHolder> map){
        ...
        if(assignment[1].contains("[") && assignment[1].contains("]")){
            ...
            int val = map.get(name2).assigned.getOrDefault(index2,-1);
            if(val < 0){
                return false;
            }
            map.get(name1).assigned.put(index1,val);
        } else {
            ...
            map.get(name1).assigned.put(index1,Integer.parseInt(assignment[1]));
        }
        return true;
    }
    public static boolean isValidCommand(String command, HashMap<Character,ArrayPlaceHolder> map){
        ...
        if(parts.length == 1) {
            ...
            ArrayPlaceHolder temp = new ArrayPlaceHolder(size,new HashMap<>());
            map.put(name,temp);
            return true;
        }
